...
for j = 1:N+1
    ...
    if(p_D) % Check if target landmark is inside the fov
        for i = j%1:P_j.Length % Add the p_D to check if the cluster is out of view
            ...
            W=(beta_0^alpha_0*gamma(alpha_k))/(beta_k^alpha_k*gamma(alpha_0)) * (norm(S_0)^(v_0/2)*gamma2(v_k/2))/(pi^(c_k-1)*sqrt(c_k)*gamma2(v_0/2)*norm(S_k)^(v_k/2));
             if(W~=0)
                 W_k(j) = W_k(j)  * W;
             end
            end
